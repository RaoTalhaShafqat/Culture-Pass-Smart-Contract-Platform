...
contract CulturePassAdminIntegration is Test, CodeConstants {
    ...
    function setUp() public {
        ...
        (cPass, ticketToken) = deployer.deploy(safeAddr);
    }
    ...
    function testRegisterVenueInvalidWallet() public {
        ...
        _execExpectFail(
            data,
            CulturePass.CulturePass__invalidWalletAddress.selector
        );
    }
    ...
    function testRegisterVenueInvalidEntryPrice() public {
        ...
        _execExpectFail(
            data,
            CulturePass.CulturePass__invalidEntryPrice.selector
        );
    }
    ...
    function testRegisterVenueDuplicate() public {
        ...
        _execExpectFail(
            data,
            CulturePass.CulturePass__venueAlreadyExists.selector
        );
    }
    ...
    function testSetPassTier() public {
        bytes memory data = abi.encodeCall(
            CulturePass.setPassTier,
            (CulturePass.Tier.Explorer, 30, 0.01 ether, 3)
        );

        _exec(data);

        (uint256 tickets, uint256 price, uint256 cap) = cPass.s_passTiers(
            CulturePass.Tier.Explorer
        );
